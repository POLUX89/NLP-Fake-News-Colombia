...
│   └── push_to_hub.py          # Phase 4: publish model + card to the HF Hub
├── notebooks/
...
├── app/                        # Streamlit demo (loads the model from the Hub)
├── docs/                       # MODEL_CARD · DATASHEET · DATA_STATEMENT · HUB_MODEL_CARD
└── tests/
```
...
# Phase 4 — publish to the HF Hub (needs `hf auth login`) and run the demo
python -m fake_news_co.push_to_hub --dry-run
